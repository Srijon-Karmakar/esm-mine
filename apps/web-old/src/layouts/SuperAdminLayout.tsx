...
const links = [
  ...
  { to: "/superadmin/clubs", label: "Clubs" },
  { to: "/superadmin/users", label: "Users" },
  { to: "/superadmin/access", label: "Access" },
];
          ...
          <aside className={`${open ? "block" : "hidden"} lg:block lg:sticky lg:top-6`}>
            <NeoCard className="p-4">
              <div className="px-2 py-3">
                ...
                <div className={`text-xs ${neo.muted}`}>System Admin</div>

                <div className={`mt-3 ${neo.inset} px-3 py-2 text-xs`}>
                  {(user as any)?.email || "superadmin"}
                </div>

                <div className="mt-2 text-[11px] text-slate-500">
                  Club: <span className="font-semibold">esportm-system</span>
                </div>
              </div>
